...
def plot_unassigned_values(normalized_microbiome_data, metadata, output_dir, substring_to_find='Unassigned', group='sample-type', palette='pastel'):
    ...
    if len(columns_with_substring) > 0:
        ...
        #     output_file = os.path.join(output_dir, f'{substring_to_find.lower()}_across_{group}s_statistics_results.csv')
        ...
        if results!=None:
            ...
            results_df.to_csv(os.path.join(output_dir, f'{substring_to_find.lower()}_across_{group}s_statistics_results.csv'))
